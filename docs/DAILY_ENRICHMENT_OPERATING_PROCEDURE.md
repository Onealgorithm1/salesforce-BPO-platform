...
## Automation note (Sprint 35)
Both the manual callout-before-DML path (this procedure) AND `OA_EnrichmentQueueable`/`OA_EnrichmentOrchestrator` **commit mode are now safe** — the orchestrator write defect is fixed (two-phase callouts-then-writes; writes ALL Leads, verified 5/5). See `AUTOMATED_WRITE_PATH_FIX.md`. Rollback is fixed (`ROLLBACK_DEFECT_FIX.md`): every write is fully reversible via `OA_ChangeLogService.rollback([logs])`. Always **verify `active policies = 0` after deactivating** (use explicit quoted `--source-dir` paths). Scheduled writes still gated on the least-privilege user + monitoring.
